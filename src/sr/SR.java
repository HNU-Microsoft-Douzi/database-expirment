...
class SR {
    ...
    void send(byte[] content) throws IOException {
        int sendIndex = 0, length; // sendIndex is used to simulate send number in tcp stream
        final int MAX_LENGTH = 1024;
        DatagramSocket datagramSocket = new DatagramSocket(ownPort); // resend datagram to localhost's direct port
        ...
        do {
            while (timers.size() < WindowSize && sendIndex < content.length && sendSeq < 256) { // until the window is run up
                ...
                byte[] temp = new byte[1]; // simulate seq in tcp, just for 8 bits
            ...
            datagramSocket.setSoTimeout(1000); // when client send picture1 to server, wait for 1 minute and then ready to receive picture3 from server
